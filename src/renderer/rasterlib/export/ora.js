...
export class ExportOra {
    ...
    process(filename) {
        ...
        let traverse = async (group, current_stack, level) => {
            for (let i = group.length - 1; i >= 0; i --) {
                ...
                if (layer instanceof LayerGroup) {
                    ...
                    ch("<layer>").attr({
                        name: layer_name, x: 0, y: 0, src: "data/"+layer_name+".png", opacity: 1.0, "composite-op": layer.compositor
                    }).appendTo(current_stack);
        ...
        traverse(this.image.layers, image_struct.find("stack"), "0").then(()=>{
            let export_merged = async () =>{
                ...
                await new Promise((resolve, reject)=>{
                    ...
                    let src  = gegl.node(top_node, {operation: 'gegl:buffer-source', buffer: this.image.buffer});
                    let save = gegl.node(top_node, {operation: 'gegl:png-save', path: temp_name});
                ...
                await new Promise((resolve, reject)=>{
                    ...
                    if (this.image.width > this.image.height) {
                        thumb_x = 256;
                        thumb_y = 256 * this.image.height / this.image.width;
                    } else {
                        thumb_y = 256;
                        thumb_x = 256 * this.image.width / this.image.height;
                    }
